...
class Policy:
  ...
  def selectAction(self, state):
    ...
    if np.random.random() < self.epsilon:
      for x in range(len(self.actionJointControl)):
        tmp = np.random.random()
        if tmp < (1/3.0):
          a.append(self.actionJointControl[x][0])
        elif tmp > (2/3.0):
          a.append(self.actionJointControl[x][2])
        else:
          a.append(self.actionJointControl[x][1])
